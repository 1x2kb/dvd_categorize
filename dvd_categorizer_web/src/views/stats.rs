use crate::components::{ActorChart, GenreChart, StatsOverviewCard, YearChart};
use dioxus::prelude::*;
use models::{BarChartData, PieChartData, StatsOverview};

#[component]
pub fn Stats() -> Element {
    let mut overview = use_signal(
        || StatsOverview {
            total_movies: 0,
            total_directors: 0,
            total_actors: 0,
        ...
        move || {
            spawn(
                async move {
                    ...
                    // Fetch year data
                    if let Ok(resp) =
                        reqwest::get("http://localhost:3000/stats/movies-by-year").await
                    {
                        if let Ok(data) = resp
                            .json::<BarChartData>()
                            .await
