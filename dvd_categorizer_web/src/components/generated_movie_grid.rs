...
pub struct GeneratedMovieGridProps {
    ...
    pub pending_inputs: ReadSignal<Vec<(String, NeedsInputReason)>>,
    ...
    pub on_pending: EventHandler<(String, NeedsInputReason)>,
...
pub fn GeneratedMovieGrid(props: GeneratedMovieGridProps) -> Element {
    ...
    let on_toast_pending = props.on_toast.clone();
    let on_dismiss_pending = props.on_dismiss_toast.clone();
    ...
    let on_toast_auto = props.on_toast.clone();
    let on_dismiss_auto = props.on_dismiss_toast.clone();
    let model_auto = props.model.clone();
    let on_error_auto = props.on_error.clone();
    let on_loading_auto = props.on_loading.clone();
    use_effect(move || {
        let snapshot = entries.read(); // reactive
        let has_suggestion = snapshot.iter().any(|e| matches!(e.pending, Some(PendingReason::Suggestion(_))));
        let resolved: Vec<(usize, String)> = snapshot.iter().enumerate()
            .filter_map(|(i, e)| if let Some(PendingReason::Resolved(t)) = &e.pending { Some((i, t.clone())) } else { None })
            .collect();
        drop(snapshot);
        if !has_suggestion && !resolved.is_empty() {
            ...
            on_toast_auto.call(ToastMessage {
                id,
                kind: ToastKind::Info,
                message: format!("Generating {} pending title{}…", resolved.len(), if resolved.len() == 1 { "" } else { "s" }),
                duration_ms: 3000,
            });
            ...
            spawn(async move {
                ...
                let titles: Vec<String> = resolved.iter().map(|(_, t)| t.clone()).collect();
                let positions: Vec<usize> = resolved.iter().map(|(i, _)| *i).collect();
    ...
    let on_title_corrected = props.on_title_corrected.clone();
    ...
    use_effect(move || {
        ...
        entries.with_mut(|v| {
            ...
            let new_v: Vec<MovieEntry> = input_titles_regen.iter().enumerate().map(|(pos, t)| {
                if let Some(existing) = v.iter().find(|e| e.input_title.to_lowercase() == t.to_lowercase()) {
                    ...
                    MovieEntry::new(t.clone(), placeholder)
                }
            }).collect();
            *v = new_v;
        });
        ...
        spawn(async move {
            ...
            let positions: Vec<usize> = to_generate.iter().map(|(i, _)| *i).collect();
            stream_generated_movies(
                titles,
                positions,
                model,
                move |movie, pending| {
                    ...
                    if let Some(reason) = pending {
                        ...
                        if let Some(tid) = old_id { on_dismiss_cb.call(tid); }
                        let all_pending_titles: Vec<String> = entries.peek().iter()
                            .filter(|e| matches!(e.pending, Some(PendingReason::Suggestion(_))))
                            .map(|e| format!("\"{}\"", e.input_title))
                            .collect();
                        let new_id = *next_toast_id.peek();
                        next_toast_id.with_mut(|n| *n += 1);
                        pending_toast_id.set(Some(new_id));
                        on_toast_cb.call(ToastMessage {
                            id: new_id,
                            kind: ToastKind::Warning,
                            message: format!("Needs input: {}", all_pending_titles.join(", ")),
                            duration_ms: 0,
                        });
                        on_pending.call((movie.input_title.unwrap_or(movie.title), reason));
                        return;
                    }
