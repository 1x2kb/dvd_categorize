...
pub mod inner {
    ...
    impl MongoMovieRepository {
        ...
        pub async fn from_env() -> DbResult<Self> {
            let mongodb_uri = env::var("MONGODB_URI").map_err(
                |_| {
                    DatabaseError::QueryError(
                        "MONGODB_URI environment variable must be set".to_string(),
                    )
                },
            )?;
            ...
            let qdrant_collection =
                env::var("QDRANT_COLLECTION").unwrap_or_else(|_| "movies".to_string());

            // Parse MongoDB URI to extract database name
            let db_name = Self::extract_db_name(&mongodb_uri).unwrap_or("dvd_catalog".to_string());
        ...
        async fn insert_qdrant_point(&self, mongo_id: &str, vector: Vec<f32>) -> DbResult<Uuid> {
            ...
            let operation = UpsertPointsBuilder::new(
                &self.qdrant_collection,
                vec![point],
            );

            self.qdrant
                .upsert_points(operation)
                .await
                .map_err(
                    |e| {
                        DatabaseError::QueryError(
        ...
        async fn insert_qdrant_points_bulk(
            ...
            embeddings_to_insert: Vec<(
                usize,
                Vec<f32>,
                String,
            )>, // (doc_index, embedding, movie_name)
        ) -> DbResult<()> {
            ...
            for (doc_index, embedding, movie_name) in embeddings_to_insert {
                ...
                if doc_index >= inserted_ids.len() {
                    warn!(
                        "Invalid doc_index {} for movie {}, skipping Qdrant insert",
                        doc_index, movie_name
                    );
                    continue;
                }
                ...
                points.push(point);
                debug!(
                    "Prepared Qdrant point for movie: {}",
                    movie_name
                );
            }
            ...
            if !points.is_empty() {
                info!(
                    "Bulk inserting {} points to Qdrant",
                    points.len()
                );
                let operation = UpsertPointsBuilder::new(
                    &self.qdrant_collection,
        ...
        async fn search_qdrant(&self, embedding: Vec<f32>, limit: i64) -> DbResult<Vec<String>> {
            ...
            debug!(
                "Searching Qdrant collection '{}' with {}-dimensional vector",
                self.qdrant_collection,
                embedding.len()
            );
            ...
            for point in &response.result {
                    ...
                    .get("movie_id")
                {
                    ...
                    None => {
                        warn!(
                            "Point {:?} has no movie_id in payload",
                            point.id
                        );
                    }
                }
    ...
    impl GetMovieById for MongoMovieRepository {
        async fn get_by_id(&self, id: Self::Id) -> DbResult<FullMovie> {
            let oid = ObjectId::parse_str(&id).map_err(
                |e| {
                    DatabaseError::QueryError(
                        format!(
                            "Invalid movie id '{}': {}",
    ...
    impl InsertMovie for MongoMovieRepository {
        async fn insert(&self, movie: FullMovie) -> DbResult<FullMovie> {
            ...
            let id = self
                .insert_movie_with_embedding(doc)
                .await?;
            self.get_by_id(id)
                .await
        }
    ...
    impl InsertMovies for MongoMovieRepository {
        ...
        > {
            ...
            let movies_with_embeddings: Vec<(
                NewMovie,
                Option<Vec<f32>>,
            )> = movies
                .iter()
                ...
                .collect();
            self.insert_batch_with_embeddings(movies_with_embeddings)
                .await
        }
    }
    ...
    impl InsertMoviesWithEmbeddings for MongoMovieRepository {
        ...
        > {
            ...
            info!(
                "Starting bulk insert of {} movies with embeddings",
                movies.len()
            );

            // Step 1: Check for existing movies by name (duplicate detection)
            let movie_names: Vec<String> = movies
                .iter()
                .map(
                    |(m, _)| {
                        m.name
            ...
            if !existing_names.is_empty() {
                info!(
                    "Found {} existing movies, skipping duplicates",
                    existing_names.len()
                );
            }
            ...
            info!(
                "Inserting {} new movies to MongoDB",
                new_docs.len()
            );
            ...
            for (idx, bson_id) in &insert_result.inserted_ids {
                let hex = bson_id
                    ...
                    .ok_or_else(
                        || {
                            DatabaseError::QueryError(
                                "MongoDB did not return an ObjectId for an inserted movie"
                                    .to_string(),
                            )
                        },
                    )?;
            ...
            let inserted_ids: Vec<String> = new_docs
                .iter()
                .map(
                    |d| {
                        d.id.clone()
            ...
            info!(
                "Successfully inserted {} movies to MongoDB",
                inserted_ids.len()
            );
            ...
            if !embeddings_to_insert.is_empty() {
                debug!(
                    "Inserting {} embeddings to Qdrant",
                    embeddings_to_insert.len()
                );
                self.insert_qdrant_points_bulk(
                    &inserted_ids,
                    &new_docs,
            ...
            info!(
                "Bulk insert completed: {} movies inserted",
                results.len()
            );
            Ok(results)
        }
    ...
    impl UpdateMovieLocation for MongoMovieRepository {
        async fn update_location(&self, movie_id: Self::Id, location: String) -> DbResult<()> {
            let oid = ObjectId::parse_str(&movie_id).map_err(
                |e| {
                    DatabaseError::QueryError(
                        format!(
                            "Invalid movie id '{}': {}",
    ...
    impl ChatSessions for MongoMovieRepository {
        ...
        async fn save_chat_message(&self, msg: NewChatMessage) -> DbResult<()> {
            let collection = self
                .db
                .collection::<NewChatMessage>("chat_messages");
            collection
                .insert_one(&msg)
                .await
                .map_err(
            ...
            let update =
                doc! { "$set": { "updated_at": Bson::String(chrono::Utc::now().to_rfc3339()) } };
            ...
            let directors = self
                ...
                .map_err(
                    |e| {
                        DatabaseError::QueryError(
                            format!(
                                "MongoDB distinct directors error: {}",
                                e
                            ),
                        )
                    },
