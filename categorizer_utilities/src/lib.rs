...
mod tests {
    ...
    fn strip_punctuation_removes_hyphens() {
        assert_eq!(strip_punctuation("Spider-Man"), "Spider Man");
        assert_eq!(strip_punctuation("X-Men"), "X Men");
    }
    ...
    fn strip_punctuation_removes_colons_and_exclamation() {
        assert_eq!(strip_punctuation("RoboCop: The Movie!"), "RoboCop The Movie");
        assert_eq!(strip_punctuation("Die Hard: With a Vengeance"), "Die Hard With a Vengeance");
    }

    #[test]
    fn strip_punctuation_normalizes_whitespace() {
        assert_eq!(strip_punctuation("Too   many    spaces"), "Too many spaces");
        assert_eq!(strip_punctuation("  Leading and trailing  "), "Leading and trailing");
    }

    #[test]
    fn strip_punctuation_handles_mixed_punctuation() {
        assert_eq!(strip_punctuation("The Good, the Bad & the Ugly"), "The Good the Bad the Ugly");
        assert_eq!(strip_punctuation("(500) Days of Summer"), "500 Days of Summer");
    }

    #[test]
    fn strip_punctuation_preserves_alphanumeric() {
        assert_eq!(strip_punctuation("2001 A Space Odyssey"), "2001 A Space Odyssey");
        assert_eq!(strip_punctuation("Se7en"), "Se7en");
    }
