...
async fn extract_entities(
        ...
        .chain(all_directors.iter())
    {
        ...
        if name_parts.len() > 1 {
            ...
            let mut all_parts_found = name_parts
                .iter()
                .all(
                    |&part| {
...
> {
    ...
    let total_movies = movies.len();
    info!(
        "Starting parallel processing of {} movies",
        total_movies
    );
    ...
    for task in tasks {
        ...
        if processed % log_interval == 0 || processed == total_movies {
            ...
            info!(
                "Processed {}/{} movies ({}%)",
                processed, total_movies, progress
            );
        }
    }
...
async fn search_movies_by_text(query: &str) -> Result<Vec<FullMovie>, String> {
    ...
    let mut results = process_movies_parallel(
        all_movies, criteria,
    )
    .await?;
